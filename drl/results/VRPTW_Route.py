...
from environments.VRPTW_Route_Environment import VRPTW_Route_Environment
...
if __name__ == "__main__":
    ...
    wrappable_env = VRPTW_Route_Environment(args.instance, config.data_dir, seed=random.randint(0, 100))
    N_ENVS = 4
    ...
    config.environment = VRPTW_Route_Environment(args.instance, config.data_dir, seed=config.seed)
    config.eval_environment = VRPTW_Route_Environment(args.instance, config.data_dir, seed=config.seed)
    ...
    config.is_vec_env = (not isinstance(config.environment, VRPTW_Route_Environment))
    ...
    if not config.linear_route: exp_name += f'_GRU_Route_{AGENTS[0].agent_name}'
